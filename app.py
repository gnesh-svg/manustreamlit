...
def update_metrics(original_gray, processed_final):
...
uploaded_file = st.sidebar.file_uploader("📂 Load Image", type=["jpg", "jpeg", "png", "tif", "bmp"])

st.sidebar.markdown("### 1. Noise Filter Strategy")
...
st.sidebar.markdown("### 2. Binarization Strategy")
thresh_type = st.sidebar.selectbox("Strategy", 
...
st.sidebar.markdown("### 3. Canny Sensitivity")
...
if uploaded_file is not None:
    ...
    with col1:
        ...
        st.image(original_bgr, channels="BGR", use_container_width=True)
    
    with col2:
        st.subheader("Enhanced Gray")
        st.image(filtered, use_container_width=True)
        
    with col3:
        st.subheader("Final Binary")
        st.image(final, use_container_width=True)

    # Metrics
    mse, psnr, ssim_val = update_metrics(gray, final)
    st.sidebar.info(f"**Metrics**\n\nMSE: {mse:,.1f}\n\nPSNR: {psnr:,.1f} dB\n\nSSIM: {ssim_val:,.3f}")

    # Save/Download Button
    res, img_encoded = cv2.imencode(".png", final)
    st.sidebar.download_button("💾 Download Final Image", data=img_encoded.tobytes(), file_name="processed_manuscript.png", mime="image/png")
else:
